...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['login'])) {
        $inspector_id = trim($_POST['id'] ?? '');
        $name = trim($_POST['name'] ?? '');
        $password = trim($_POST['password'] ?? '');

        if (!empty($inspector_id) && !empty($name) && !empty($password)) {
            if ($stmt = $conn->prepare("SELECT inspector_id, name, password FROM inspectors WHERE inspector_id = ? AND name = ?")) {
                $stmt->bind_param("ss", $inspector_id, $name);
                $stmt->execute();
                $stmt->store_result();
        ...
        if (!empty($email)) {
            if ($stmt = $conn->prepare("SELECT name FROM inspectors WHERE email = ?")) {
...
<head>
    ...
    <style>
        body {
            ...
            height: 100vh;
            margin: 0;
            background: url('B8.jpg') no-repeat center center fixed;
        ...
        /* Use the same styles as in farmer_login.php */
    </style>
</head>
<body>
    <div class="container">
        <div class="login-box">
            ...
            <form action="inspector_login.php" method="POST">
                <div class="textbox">
                    <input type="text" name="id" placeholder="Inspector ID" required>
                </div>
                <div class="textbox">
                ...
                <p>New User? <a href="inspector_reg.html" id="register-link">Register here</a></p>
    ...
    <script>
        ...
        // Use the same script as in farmer_login.php
    </script>
</body>
</html>
